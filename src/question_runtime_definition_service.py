...
class QuestionRuntimeDefinitionService:
    ...
    def delete_group_signal(self, definition: dict[str, Any], group_id: str, signal_ref: str) -> RuntimeSignalDefinition:
        normalized = normalize_runtime_definition(definition)
        normalized['extended_signal_groups'] = _with_group_signal_deleted(
            normalized['extended_signal_groups'],
            group_id,
            signal_ref,
        )
    ...
    def _target_path(self, trait_id: str, trait_name: str) -> Path:
        ...
        filename = f'{runtime_trait_id}.json'
        return self.traits_dir / filename
...
def build_runtime_trait_id(
    trait_id: str,
    *,
    trait_name: str,
    existing_runtime_trait_id: Any = '',
) -> str:
...
def _normalize_group(group: dict[str, Any]) -> RuntimeSignalGroup:
    ...
    return {
        'group_id': _normalize_group_id(source.get('group_id')),
...
def _replace_signal(
    signals: list[RuntimeSignalRecord],
    signal_ref: str,
    updates: dict[str, Any],
    *,
    default_group: str,
    ...
    for signal in signals:
        if signal['ref'] != normalized_ref:
            ...
            continue
        merged = {**signal, **updates}
        updated_signals.append(_normalize_signal(merged, default_group=default_group))
        replaced = True
    if replaced:
...
def _replace_group(groups: list[RuntimeSignalGroup], group_id: str, updates: dict[str, Any]) -> list[RuntimeSignalGroup]:
    ...
    for group in groups:
        if group['group_id'] != normalized_group_id:
            ...
            continue
        merged = {**group, **updates}
        updated_groups.append(_normalize_group(merged))
        replaced = True
    if replaced:
...
def _with_group_signal_updated(
    groups: list[RuntimeSignalGroup],
    group_id: str,
    signal_ref: str,
    updates: dict[str, Any],
) -> list[RuntimeSignalGroup]:
    normalized_group_id = _normalize_group_id(group_id)
    updated_groups: list[RuntimeSignalGroup] = []
    replaced = False
    for group in groups:
...
def _normalize_weight(value: Any) -> float:
    if isinstance(value, bool):
        raise ValueError('Signal weight must be a non-negative number.')
    text = str(value or '0').strip()
    try:
        parsed = float(text)
    except ValueError as exc:
        raise ValueError('Signal weight must be a non-negative number.') from exc
    if parsed < 0:
        raise ValueError('Signal weight must be a non-negative number.')
    return parsed
