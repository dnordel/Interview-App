...
def test_history_controller_leaves_existing_records_unchanged_when_regeneration_is_requested(tmp_path, monkeypatch) -> None:
    calls: list[str] = []
    app = SimpleNamespace(
        settings={"base_dir": str(tmp_path)},
        history_store=SimpleNamespace(path=tmp_path / "interview_history.sqlite3"),
    ...
    assert calls == [
        "warning:Regeneration is unavailable for existing interview records. Finalized reports remain unchanged."
    ]
