...
def test_resolve_default_windows_system_device_falls_back_to_preferred_when_probe_empty():
    assert resolve_default_windows_system_device(device_probe=lambda: []) == "VB-Audio Virtual Cable (CABLE Input)"


def test_resolve_default_windows_microphone_device_uses_available_local_mic():
    probe = lambda: ["Microphone Array (Intel Smart Sound Technology)", "CABLE Output (VB-Audio Virtual Cable)"]
    assert resolve_default_windows_microphone_device(device_probe=probe) == "Microphone Array (Intel Smart Sound Technology)"
