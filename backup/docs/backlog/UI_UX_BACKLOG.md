...
| UX-B002 | UX-F002 | 13 | Admin users misconfigure settings due to mixed-priority controls in one view | `docs/UI_UX_RECOMMENDATIONS_AND_TASK_STUBS.md` section 2 | Split settings into tabs and add section-level validation + advanced defaults reset | `src/ui_windows.py`, `src/interview_app.pyw` | Eng + UX | Done |
...
| UX-B004 | UX-F004 | 14 | Dashboard awareness does not quickly convert into action | `docs/UI_UX_RECOMMENDATIONS_AND_TASK_STUBS.md` section 4 | Add clickable KPI chips and recommended next action banner | `src/onboarding_app.pyw`, `src/onboarding_task_filters.py` | Eng + UX | Done |
